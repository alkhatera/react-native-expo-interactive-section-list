...
import { View, SectionList, Text, StyleSheet, ImageSourcePropType, SectionListRenderItem, SectionListData } from 'react-native';
...
interface Props {
	...
	renderSeparatorComponent: () => JSX.Element;
...
const InteractiveSectionList: React.FC<Props> = ({
	...
	const handleRenderSectionHeader: any = (info: { section: SectionListData<any> }) => {
		...
		return <Text style={styles.sectionHeader}>{info.section.title}</Text>;
	};
		...
		<SectionList
			...
			onScroll={({ nativeEvent }) => {
				...
				const itemIndex = Math.floor(
					nativeEvent.contentOffset.y / (itemHeight + ((sectionHeaderHeight + separatorHeight) * sectionsFeed.length) / flattenedList.length)
				);
